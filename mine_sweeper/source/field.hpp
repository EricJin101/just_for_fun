  ...
  switch (gameMode) {
    ...
    case CUSTOM:
      do {
        dispBanner();
        std::cout << endl;
        std::cout << white_fg << "    Minimum breadth of MineField is 9"
                  << endl;
        std::cout << white_fg << "    Maximum size of MineField is 30 x 16"
                  << endl;
    ...
    if (j != b - 1) {
      ...
      for (int k = 0; k < l - 1; ++k) {
        if (cells[k][j].hidden) {
          ...
          if (cells[k + 1][j].hidden && !cells[k][j + 1].hidden &&
              !cells[k + 1][j + 1].hidden) {
            writeBuf << "╇";
          }

          else if (!cells[k + 1][j].hidden && cells[k][j + 1].hidden &&
                   !cells[k + 1][j + 1].hidden) {
            writeBuf << "╉";
          }
...
bool Field::checkValidityOfQuickClear() {
  ...
  for (int i = x - 1; i < x + 2; ++i) {
    for (int j = y - 1; j < y + 2; ++j) {
      if (i < 0 || i > l - 1 || j < 0 || j > b - 1) continue;

      if (i == x && j == y) continue;

      if (cells[i][j].flagged) ++flagsCopy;
    }
  }
...
void Field::startSweep(int x, int y) {
  ...
  if (!cells[x][y].hidden && !QUICKCLEAR) return;
